...
#include "flash_fwd_kernel_half_qi8.h"
#include "flash_fwd_kernel_full_qi8.h"
...
DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_kernel_half_qi8, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Return_softmax) {
    #if defined(ARCH_SUPPORTS_FLASH)
        static_assert(!(Is_causal && Is_local)); // Enforce constraints
        flash::compute_attn_half_qi8<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Return_softmax>(params);
    #else
        FLASH_UNSUPPORTED_ARCH
    #endif
}

DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_kernel_full_qi8, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Return_softmax) {
    #if defined(ARCH_SUPPORTS_FLASH)
        static_assert(!(Is_causal && Is_local)); // Enforce constraints
        flash::compute_attn_full_qi8<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Return_softmax>(params);
    #else
        FLASH_UNSUPPORTED_ARCH
    #endif
}
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal>
void run_flash_fwd_half_qi8(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr size_t smem_size = Kernel_traits::kSmemSize;
    // printf("smem_size = %d\n", smem_size);
            ...
            LOCAL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !Is_causal, Is_local, [&] {
                BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                    ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                        ...
                        // If head dim > 128, set IsEvenMNConst to false to reduce number of templates
                        // If Is_local, set Is_causal to false
                        auto kernel = &flash_fwd_kernel_half_qi8<Kernel_traits, Is_dropout, Is_causal, Is_local && !Is_causal, Has_alibi, IsEvenMNConst && IsEvenKConst && !Is_local && !ReturnSoftmaxConst && Kernel_traits::kHeadDim <= 128, IsEvenKConst, ReturnSoftmaxConst && Is_dropout>;
                        // auto kernel = &flash_fwd_kernel<Kernel_traits, false, Is_causal, false, false, true, true, false>;
                        // printf("IsEvenMNConst = %d, IsEvenKConst = %d, Is_local = %d, Is_causal = %d, ReturnSoftmaxConst = %d, Is_dropout = %d\n", int(IsEvenMNConst), int(IsEvenKConst), int(Is_local), int(Is_causal), int(ReturnSoftmaxConst), int(Is_dropout));
                        // auto kernel = &flash_fwd_kernel<Kernel_traits, false, Is_causal, false, true, true, false>;
                        if (smem_size >= 48 * 1024) {
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal>
void run_flash_fwd_full_qi8(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr size_t smem_size = Kernel_traits::kSmemSize;
            ...
            LOCAL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !Is_causal, Is_local, [&] {
                BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                    ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                        ...
                        // If head dim > 128, set IsEvenMNConst to false to reduce number of templates
                        // If Is_local, set Is_causal to false
                        auto kernel = &flash_fwd_kernel_full_qi8<Kernel_traits, Is_dropout, Is_causal, Is_local && !Is_causal, Has_alibi, IsEvenMNConst && IsEvenKConst && !Is_local && !ReturnSoftmaxConst && Kernel_traits::kHeadDim <= 128, IsEvenKConst, ReturnSoftmaxConst && Is_dropout>;
                        // auto kernel = &flash_fwd_kernel<Kernel_traits, false, Is_causal, false, false, true, true, false>;
                        // printf("IsEvenMNConst = %d, IsEvenKConst = %d, Is_local = %d, Is_causal = %d, ReturnSoftmaxConst = %d, Is_dropout = %d\n", int(IsEvenMNConst), int(IsEvenKConst), int(Is_local), int(Is_causal), int(ReturnSoftmaxConst), int(Is_dropout));
                        // auto kernel = &flash_fwd_kernel<Kernel_traits, false, Is_causal, false, true, true, false>;
                        if (smem_size >= 48 * 1024) {
...
template<typename T>
void run_mha_fwd_hdim128_half_qi8(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int Headdim = 128;
    auto dprops = at::cuda::getCurrentDeviceProperties();
    bool is_sm8x = dprops->major == 8 && dprops->minor > 0;
    DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
            ...
            if (is_sm8x) {
                if constexpr(!Is_causal) {
                    run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
                } else {
                    // run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 64, 64, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
                    run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
                    // run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 256, 64, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
                    // run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 64, 128, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
                    // run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 32, 128, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
                }
            } else {
                run_flash_fwd_half_qi8<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T, true>, Is_dropout, Is_causal>(params, stream);
            }
...
template<typename T>
void run_mha_fwd_hdim128_full_qi8(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr static int Headdim = 128;
    auto dprops = at::cuda::getCurrentDeviceProperties();
    bool is_sm8x = dprops->major == 8 && dprops->minor > 0;
    DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
            ...
            if (is_sm8x) {
                if constexpr(!Is_causal) {
                    run_flash_fwd_full_qi8<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T, true, true>, Is_dropout, Is_causal>(params, stream);
                } else {
                    run_flash_fwd_full_qi8<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T, true, true>, Is_dropout, Is_causal>(params, stream);
                }
            } else {

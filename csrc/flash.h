...
struct Qkv_params {
    ...
    bool is_qk_quant = false;
    bool is_v_quant = false;
    // The QKV matrices.
    ...
    // scale per token
    void *__restrict__ q_scale_ptr = nullptr;
    ...
    // The stride between rows of the Q_scale and K_scale matrices.
...
template<typename T, int Headdim> void run_mha_fwd_half_qi8_(Flash_fwd_params &params, cudaStream_t stream);
template<typename T, int Headdim> void run_mha_fwd_full_qi8_(Flash_fwd_params &params, cudaStream_t stream);

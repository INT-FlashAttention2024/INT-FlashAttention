...
template <typename To_type, typename Engine, typename Layout>
__forceinline__ __device__ auto convert_type_tensor(Tensor<Engine, Layout> const &tensor) {
    Tensor ret = make_tensor<To_type>(tensor.layout());
    // HACK: this requires tensor to be "contiguous"
    return make_tensor(make_rmem_ptr<To_type>(ret.data()), tensor.layout());
}
...
int64_t resolve_thread_kv_page_slice_offset(const int tidx, const int n_block_max, const int page_block_size, 
    ...
    return int64_t(block_table[virtual_page_idx]) * page_stride 
...
template <bool Is_even_MN=true, bool Is_even_K=true, bool Clear_OOB_MN=false, bool Clear_OOB_K=true,
          typename TiledCopy, typename Engine0, typename Layout0, typename Engine1, typename Layout1>
__forceinline__ __device__ void copy_scale(TiledCopy tiled_copy, Tensor<Engine0, Layout0> const &S,
                            Tensor<Engine1, Layout1> &D,
                            const int thr_limit=0) {
    if (threadIdx.x < thr_limit){
...
template <int ele_per_thr, int thr_limit, bool Is_even_MN=true,
 typename Engine0, typename Layout0, typename Engine1, typename Layout1>
__forceinline__ __device__ void copy_simple(Tensor<Engine0, Layout0> const &S,
                            Tensor<Engine1, Layout1> &D, const int max_MN=0) {
    if (threadIdx.x < thr_limit){
        CUTE_STATIC_ASSERT_V(rank(S) == Int<1>{});
        CUTE_STATIC_ASSERT_V(rank(D) == Int<1>{});
        CUTE_STATIC_ASSERT_V(size<0>(S) == size<0>(D));  // Length
        int beg = threadIdx.x * ele_per_thr;
        const int end = Is_even_MN ? (threadIdx.x+1) * ele_per_thr : min((threadIdx.x+1) * ele_per_thr, max_MN);
        #pragma unroll
        for(; beg < end; ++beg){
            D[beg] = S[beg];
        }
        // SM80_CP_ASYNC_CACHEGLOBAL<uint128_t>::copy(S+offset, D+offset);
    }
}

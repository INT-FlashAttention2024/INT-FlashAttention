...
struct Mask {
    ...
    template <bool Causal_mask=false, bool Is_even_MN=true, typename Engine, typename Layout, 
                typename Enginei, typename Layouti, typename Engine2, typename Layout2, typename Engine3, typename Layout3>
    __forceinline__ __device__ void apply_rescale_mask(Tensor<Engine, Layout> &tensor_, 
                                               const Tensor<Enginei, Layouti> &tensor_i_, 
                                               Tensor<Engine2, Layout2> &scale_row,
                                               Tensor<Engine3, Layout3> &scale_col,
                                               const int col_idx_offset_,
                                               const int row_idx_offset,
                                               const int warp_row_stride,
                                               const int col_scale_bias,
                                               const int row_scale_bias) {
        static_assert(!(Causal_mask && Is_local), "Cannot be both causal and local");
        static_assert(Layout::rank == 3, "Only support 3D Tensor");
        static_assert(Layouti::rank == 3, "Only support 3D Tensor");
        static_assert(Layout2::rank == 1, "Scale Array Only support 1D Tensor");
        static_assert(Layout3::rank == 1, "Scale Array Only support 1D Tensor");
        static_assert(decltype(size<0>(tensor_))::value == 4, "First dimension must be 4");
        static constexpr bool Need_masking = Has_alibi || Causal_mask || Is_local || !Is_even_MN;
        // if (cute::thread0()) { printf("Has_alibi = %d, Causal_mask=%d, Is_local=%d, Is_even_MN = %d, Need_masking = %d\n", Has_alibi, Causal_mask, Is_local, Is_even_MN, Need_masking); }
        ...
        // Reshape tensor_ from (MMA=4, MMA_M, MMA_N) to (nrow=(2, MMA_M), ncol=(2, MMA_N))
        Tensor tensor = make_tensor(tensor_.data(), flash::convert_layout_acc_rowcol(tensor_.layout()));
        Tensor tensor_i = make_tensor(tensor_i_.data(), flash::convert_layout_acc_rowcol(tensor_i_.layout()));
        // Do we need both row and column indices, or just column incides?
        const int lane_id = threadIdx.x % 32;
        const int col_idx_offset = col_idx_offset_ + (lane_id % 4) * 2;
        if(Need_masking){
                        ...
                        for (int j = 0; j < size<1, 0>(tensor); ++j) {
                            ...
                            if constexpr (Has_alibi) {
                                tensor(make_coord(i, mi), make_coord(j, nj)) = 
                                        scale_row[row_idx-row_scale_bias] * scale_col[col_idx-col_scale_bias] * tensor_i(make_coord(i, mi), make_coord(j, nj));
                                if constexpr (Is_causal) {
                                    tensor(make_coord(i, mi), make_coord(j, nj)) += alibi_slope * col_idx;
                                } else {
                                    tensor(make_coord(i, mi), make_coord(j, nj)) -= alibi_slope * abs(row_idx + max_seqlen_k - max_seqlen_q - col_idx);
    ...
    template <typename Engine, typename Layout, 
                typename Enginei, typename Layouti, typename Engine2, typename Layout2>
    __forceinline__ __device__ void apply_rescale_v(Tensor<Engine, Layout> &tensor_, 
                                               const Tensor<Enginei, Layouti> &tensor_i_, 
                                               Tensor<Engine2, Layout2> &scale_col) {
        static_assert(Layout::rank == 3, "Only support 3D Tensor");
        static_assert(Layouti::rank == 3, "Only support 3D Tensor");
        static_assert(Layout2::rank == 1, "Scale Array Only support 1D Tensor");
        static_assert(decltype(size<0>(tensor_))::value == 4, "First dimension must be 4");
        ...
        for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
            ...
            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                ...
                #pragma unroll
                for (int mi = 0; mi < size<0>(tensor); ++mi) {
                    tensor(mi, make_coord(j, nj)) += scale_col[col_idx] * tensor_i(mi, make_coord(j, nj)) / 127.0f;
                }
            }
        }
    };
    ...
    template <typename Engine, typename Layout>
    __forceinline__ __device__ void print_s(Tensor<Engine, Layout> &tensor_,
                                               const int col_idx_offset_,
                                               const int row_idx_offset,
                                               const int warp_row_stride) {
        // Reshape tensor_ from (MMA=4, MMA_M, MMA_N) to (nrow=(2, MMA_M), ncol=(2, MMA_N))
        ...
        for (int mi = 0; mi < size<0, 1>(tensor); ++mi) {
            ...
            for (int i = 0; i < size<0, 0>(tensor); ++i) {
                ...
                for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                    ...
                    for (int j = 0; j < size<1, 0>(tensor); ++j) {
                        const int col_idx = col_idx_base + j;
                        printf("row_idx = %d, col_idx = %d, val = %d\n", row_idx, col_idx, tensor(make_coord(i, mi), make_coord(j, nj)));
                    }
                }
            }
        }
